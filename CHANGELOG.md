...
- **Strict Config Validation (#20)**: unknown keys in `epazote.yml` now fail at startup with a clear error instead of being silently ignored (e.g. `max_size` reports `unknown field 'max_size', expected one of ... 'max_bytes' ...`). Review your config for stray keys before upgrading.
